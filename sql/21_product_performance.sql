-- Phase 4: product-level performance preview.
-- Grain: one row per sold product. Read-only; shows the 50 products with
-- the highest recognized revenue in completed calendar months.
-- Gross profit is estimated as item sale price minus current product cost.
...
SELECT
  ...
  ROUND(estimated_product_cost, 2) AS estimated_product_cost,
...
ORDER BY overall_revenue_rank, product_id
LIMIT 50;
